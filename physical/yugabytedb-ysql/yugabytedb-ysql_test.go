...
import (
	...
	YSQL "github.com/hashicorp/vault/helper/testhelpers/yugabytedb-ysql"
...
func TestYSQLBackend(t *testing.T) {
	logger := logging.NewVaultLogger(log.Debug)

	connURL := os.Getenv("YBDBURL")
	if connURL == "" {
		cleanup, u := YSQL.PrepareTestContainer(t, "latest")
		defer cleanup()
		connURL = u
	}

	table := os.Getenv("YBTABLE")
	if table == "" {
		table = "vault_kv_store"
	}

	hae := os.Getenv("YBHAENABLED")
	if hae == "" {
		hae = "true"
	}

	b1, err := NewYSQLBackend(map[string]string{
	...
	b2, err := NewYSQLBackend(map[string]string{
		"connection_url": connURL,
		"table":          table,
		"ha_enabled":     hae,
	}, logger)
	...
	setupDatabaseObjects(t, logger, yb)

	defer func() {
		yb := b1.(*YSQLBackend)
		_, err := yb.client.Exec(fmt.Sprintf(" TRUNCATE TABLE %v ", yb.table))
		if err != nil {
	...
	if !ok {
		t.Fatalf("YSQLDB does not implement HABackend")
	}
	...
	if ha1.HAEnabled() && ha2.HAEnabled() {
		...
		testysqlSQLLockTTL(t, ha1)
		testysqlSQLLockRenewal(t, ha1)
	}
}

func TestYSQLBackendMaxIdleConnectionsParameter(t *testing.T) {
	_, err := NewYSQLBackend(map[string]string{
...
func TestConnectionURL(t *testing.T) {
	...
	for name, tt := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if tt.input.envar != "" {
				os.Setenv("VAULT_YSQL_CONNECTION_URL", tt.input.envar)
				defer os.Unsetenv("VAULT_YSQL_CONNECTION_URL")
			}
...
func testysqlSQLLockTTL(t *testing.T, ha physical.HABackend) {
	t.Log("Skipping testysqlSQLLockTTL portion of test.")
	return
...
// lock.
func testysqlSQLLockRenewal(t *testing.T, ha physical.HABackend) {
	// Get the lock
...
func setupDatabaseObjects(t *testing.T, logger log.Logger, yb *YSQLBackend) {
	var err error
	// Setup tables and indexes if not exists.
	createTableSQL := fmt.Sprintf( `CREATE TABLE IF NOT EXISTS %v (
	...
	_, err = yb.client.Exec(createTableSQL)
	if err != nil {
		t.Fatalf("Failed to create table: %v", err)
	}

	createIndexSQL := fmt.Sprintf(" CREATE INDEX IF NOT EXISTS parent_path_idx ON %v (parent_path); ", yb.table)

	_, err = yb.client.Exec(createIndexSQL)
	if err != nil {
	...
	_, err = yb.client.Exec(createHaTableSQL)
	if err != nil {
