...
- [Accessing Authority Information](#accessing-authority-information)
  ...
  - [Read Raw Issuer Certificate](#read-raw-issuer-certificate)
  - [Read Raw Issuer Certificate Chain](#read-raw-issuer-certificate-chain)
  - [Read Raw CRL](#read-raw-crl)
...
Vault now allows a single PKI mount to have multiple Certificate Authority
(CA) certificates ("issuers") in a single mount, for the purpose of
facilitating rotation. All issuers within a single mount are treated as a
single authority, meaning that Certificate Revocation List (CRL) configuration
and other authority URLs are common to all issuers and serial numbers will be
unique across all issuers. However, since each issuer may have a distinct
subject and keys, different issuers may have different CRLs.
...
Consumers of the PKI Secrets Engine mount point will have access to the
following unauthenticated APIs, useful for reading information about the
certificate authority in this mount point.

This includes information about CA certificates, their chains, and signed
CRLs, containing an encoded list of revoked certificates previously
issued by this authority.

Certain endpoints, like listing all certificates issued by this authority
or listing all issuers are authenticated, though not otherwise considered
...
~> Note: This endpoint is authenticated.
...
This endpoint retrieves the CA certificate as a standard Vault data structure
that can be read by the Vault CLI. Note that the response differs between
the legacy `/pki/cert/ca` path and the newer `/pki/issuer/:issuer_ref/json`
path; the latter includes the full `ca_chain` of the issuer, removing the need
for a separate endpoint.

These are unauthenticated endpoints.

| Method | Path                           | Issuer    |
| :----- | :----------------------------- | :-------- |
| `GET`  | `/pki/cert/ca`                 | `default` |
| `GET`  | `/pki/issuer/:issuer_ref/json` | selected  |

#### Sample Request

```shell-session
$ curl \
    http://127.0.0.1:8200/v1/pki/issuer/default/json
```
...
{
  "data": {
    "certificate": "-----BEGIN CERTIFICATE-----\nMIIDFTCCAf2gAwIBAgIUZ7T3LKrvuTD2rvUSIXmsCIqGiXIwDQYJKoZIhvcNAQEL...",
    "revocation_time": 0
  }
}
```
...
### Read Raw Issuer Certificate

This endpoint retrieves the CA certificate _in raw DER- or PEM-encoded form_.
This is a bare endpoint that does not return a standard Vault data structure
and cannot be read by the Vault CLI; [use `/pki/cert/ca` or
`/pki/issuer/:issuer_ref/json`](#read-issuer-certificate) for that. If `/pem`
is added to the endpoint, the CA certificate is returned in PEM-encoded format.

These are unauthenticated endpoints.

| Method | Path                                 | Issuer    |
| :----- | :----------------------------------- | :-------- |
| `GET`  | `/pki/ca(/pem)`                      | `default` |
| `GET`  | `/pki/issuer/:issuer_ref/(der┆pem)`  | selected  |

#### Sample Request

```shell-session
$ curl \
    http://127.0.0.1:8200/v1/pki/issuer/default/der
```
...
### Read Raw Issuer Certificate Chain

This endpoint retrieves the default issuer's CA certificate chain, including
the default issuer, _in PEM format_. This is a bare endpoint that does not
return a standard Vault data structure and cannot be read by the Vault CLI;
use `/pki/cert/ca_chain` or `/pki/issuer/:issuer_ref/json` for that.

This is an unauthenticated endpoint.

| Method | Path            | Issuer    |
| :----- | :-------------- | :-------- |
| `GET`  | `/pki/ca_chain` | `default` |

~> **Note**: Vault now returns the full chain (including the default issuer's
   certificate and all parent issuers) in the `/pki/ca_chain` response.
...
This endpoint retrieves the CRL in a standard Vault data structure that can
be read by the Vault CLI. Note that the response differs between the legacy
`/pki/cert/cert` path and the newer `/pki/issuer/:issuer_ref/crl` path;
the latter correctly places the PEM-encoded CRL in the `crl` field.

These are unauthenticated endpoints.

| Method | Path                          | Issuer    |
| :----- | :---------------------------- | :-------- |
| `GET`  | `/pki/cert/crl`               | `default` |
| `GET`  | `/pki/issuer/:issuer_ref/crl` | selected  |

#### Sample Request

```shell-session
$ curl \
    http://127.0.0.1:8200/v1/pki/issuer/default/crl
```
...
This endpoint retrieves the CRL _in raw DER- or PEM-encoded form_. This is a
bare endpoint that does not return a standard Vault data structure and cannot
be read by the Vault CLI; use `/pki/cert/crl` or `/pki/issuer/:issuer_ref/crl`
for that. If `/pem` is added to the endpoint, the CRL is returned in PEM format.

These are unauthenticated endpoints.

| Method | Path                                    | Issuer    |
| :----- | :-------------------------------------- | :-------- |
| `GET`  | `/pki/crl(/pem)`                        | `default` |
| `GET`  | `/pki/issuer/:issuer_ref/crl/(der┆pem)` | selected  |
...
The response does not include the special serial numbers (`ca`, `ca_chain`,
and `crl`).
...
~> **Note**: Vault now returns the full chain (including this certificate and
   all parent issuers) in the `ca_chain` response, for both the `certificate`
   and newer `ca_chain` fields. The root certificate is no longer elided.
...
- `serial` `(string: <required>)` - Specifies the serial number of the
  certificate to select, in hyphen-separated or colon-separated hexadecimal.
...
as well, so that only the root CA need be in a client's trust store. Choice of
issuing CA is determined first by the role (when using the `/pki/issue/:name`
path) and then by the path (when using the `/pki/issuer/:issuer_ref/issue/name`
path).
...
| `POST` | `/pki/sign/:issuer_ref/issue/:name | Path selected |
...
~> **Note**: This operation is highly privileged as it allows revocation of
   arbitrary certificates based purely on their serial number. It does not
   validate that the requesting user issued the certificate or has posession
   of the private key.
...
or import new issuer certificates and keys.
...
as a root, and for many other CAs, the various parameters on the final signed
...
| Method | Path                          | Allows private keys |
| :----- | :---------------------------- | :------------------ |
| `POST` | `/pki/config/ca`              | yes                 |
| `POST` | `/pki/issuers/import/bundle   | yes                 |
| `POST` | `/pki/issuers/import/cert     | no                  |
| `POST` | `/pki/intermediate/set-signed | no                  |
...
~> Note: this parameter is on the `/pki/issuers/import/*` and
   `/pki/intermediate/set-signed` paths; it is not on the `/pki/config/ca`
   path.
