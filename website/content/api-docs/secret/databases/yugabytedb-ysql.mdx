---
layout: api
page_title: YugabyteDB (ysql) - Database - Secrets Engines - HTTP API
description: >-
  The YugabyteDB (ysql) plugin for Vault's database secrets engine generates database
  credentials to access YugabyteDB (ysql) servers.
---

# YugabyteDB (ysql) Database Plugin HTTP API

The YugabyteDB (ysql) database plugin is one of the supported plugins for the database
secrets engine. This plugin generates database credentials dynamically based on
configured roles for the YugabyteDB (ysql) database.
...
- `connection_url` `(string: <required>)` - Specifies the YugabyteDB DSN. This field
...
```json
{
  "plugin_name": "yugabytedb-ysql-database-plugin",
  "allowed_roles": "readonly",
  "connection_url": "postgresql://{{username}}:{{password}}@localhost:5433/yugabyte",
...
$ curl \
    ...
    --request POST \
    --data @payload.json \
    http://127.0.0.1:8200/v1/database/config/yugabytedb-ysql
```
